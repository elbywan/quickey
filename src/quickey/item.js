...
export class Item {
    ...
    constructor(label: string, description: string, action: void => void) {
        this._label = label
    ...
    toString(key: string, separator: string = ': ') {
        ...
        const label = (idx < 0) ? chalk.bold(this._label) : (
            ...
            (chalk: any).bold[idx === 0 ? colors.keys.matching : colors.keys.notMatching](this._label.substring(idx, idx + 1)) +
